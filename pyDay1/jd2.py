...
with open("jd2.csv","a",encoding="utf-8",newline="") as file:
    ...
    for li in all_li:
        ...
        id = li.find_element_by_css_selector("div > div.p-price > id").text

        price = li.find_element_by_css_selector("div > div.p-price > strong > i").text
        # print(price)
        # J_comment_10056485243189
        # J_comment_10033282079592
        referenceName = li.find_element_by_css_selector("div > div.p-name.p-name-type-2 > a > em").text
        # print(name)
        referenceTime = li.find_element_by_css_selector("div > div.p-name.p-name-type-2 > time").text

        content = li.find_element_by_css_selector("div > div.p-name.p-name-type-2").get_attribute("head")
        ...
        # print(store)
        all_item.append((id,score,referenceName,referenceTime,content))     # 将数据保存到集合中去
